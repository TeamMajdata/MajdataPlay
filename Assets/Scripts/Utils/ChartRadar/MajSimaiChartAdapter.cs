...
internal sealed class AdaptationResult
{
    internal RadarChartInput? Chart { get; set; }
    internal IReadOnlyList<string> Errors { get; set; } = Array.Empty<string>();
    internal bool IsCancelled { get; set; }
    internal bool IsSuccess => Chart is not null && Errors.Count == 0 && !IsCancelled;
}

internal sealed class MajSimaiChartAdapter
{
    ...
    internal AdaptationResult Adapt(
        SimaiChart chart,
